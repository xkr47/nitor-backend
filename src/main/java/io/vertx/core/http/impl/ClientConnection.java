...
class ClientConnection extends ConnectionBase implements HttpClientConnection, HttpClientStream {
  ...
  void handleResponseEnd(LastHttpContent trailer) {
    ...
    if (currentResponse.statusCode() != 100 && requestForResponse.method() != HttpMethod.CONNECT) {

      boolean close = false;
      // See https://tools.ietf.org/html/rfc7230#section-6.3
      String responseConnectionHeader = currentResponse.getHeader(HttpHeaders.Names.CONNECTION);
      HttpVersion protocolVersion = client.getOptions().getProtocolVersion();
  ...
  private void prepareHeaders(HttpRequest request, String hostHeader, boolean chunked) {
    ...
    if (!client.getOptions().isKeepAlive() && client.getOptions().getProtocolVersion() == HttpVersion.HTTP_1_1) {
      request.headers().set(CONNECTION, CLOSE);
    } else if (client.getOptions().isKeepAlive() && client.getOptions().getProtocolVersion() == HttpVersion.HTTP_1_0) {
